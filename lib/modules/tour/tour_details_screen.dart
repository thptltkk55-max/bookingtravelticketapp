...
class TourDetailsScreen extends StatelessWidget {
  ...
  Future<void> _openDirectionsToTour() async {
    final destination = GoogleMapWidget.resolveLatLng(
      idCity: tourModel.idCity,
      location: tourModel.location,
    );
    final destinationName = tourModel.location ?? tourModel.nameTour;

    debugPrint(
      '[DIRECTIONS] destination: $destinationName '
    ...
    try {
      ...
      debugPrint('[DIRECTIONS] permission: $permission');

      if (permission) {
        final serviceEnabled =
            await geolocator.Geolocator.isLocationServiceEnabled();
        debugPrint('[DIRECTIONS] location service enabled: $serviceEnabled');
        ...
        if (serviceEnabled) {
          ...
          Get.snackbar(
            ...
            'Vui lòng bật dịch vụ vị trí để chỉ đường từ vị trí hiện tại.',
          );
        }
      } else {
        Get.snackbar(
          StringConst.notification.tr,
          'Không có quyền vị trí nên chỉ mở địa điểm tour trên Google Maps.',
        );
      }
    } catch (e, stackTrace) {
      debugPrint('[DIRECTIONS][ERROR] get current location failed: $e');
      debugPrintStack(stackTrace: stackTrace);
      Get.snackbar(
        StringConst.notification.tr,
        'Không lấy được vị trí hiện tại nên chỉ mở địa điểm tour.',
      );
    }
    ...
    debugPrint('[DIRECTIONS] launch url: $uri');
    ...
    try {
      ...
      if (!launched) {
        debugPrint('[DIRECTIONS][ERROR] launchUrl returned false');
        Get.snackbar(
          StringConst.error.tr,
          'Không mở được Google Maps. Vui lòng thử lại.',
        );
      }
    } catch (e, stackTrace) {
      debugPrint('[DIRECTIONS][ERROR] launch url failed: $e');
      debugPrintStack(stackTrace: stackTrace);
      Get.snackbar(
        StringConst.error.tr,
        'Không mở được Google Maps. Vui lòng thử lại.',
    ...
    return Column(
      ...
      children: [
        ...
        GoogleMapWidget(
          location: tourModel.location,
          idCity: tourModel.idCity,
          onDirectionsTap: _openDirectionsToTour,
        ),
